...
public class LabDetailFragment extends BaseFragment implements BaseDialogFragment.DialogClickListener {
    ...
    public static LabDetailFragment newInstance(String id, String booked) {
        LabDetailFragment fragment = new LabDetailFragment();
        mId = id;
        booked_as = booked;
        return fragment;
    }
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        getData();
        txt_pregnancy.setVisibility(View.GONE);
    ...
    public void getData(){
        ...
        apiFactory.laboratory_detail(mId)
                ...
                .subscribe(resource -> {
                    ...
                    if(resource.status) {
                        ...
                        if (data.get("case_code").getAsString().equalsIgnoreCase("no data") || TextUtils.isEmpty(data.get("case_code").getAsString())) {
                            ...
                            try {
                                txt_test.setText(data.get("test2").getAsString());
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }

                            try {
                                txt_diagnosis.setText(data.get("diagnosisl").getAsString());
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }

                            try {
                                txt_instruction.setText(data.get("instructionsl").getAsString());
                            } catch (Exception ex) {
                                ex.printStackTrace();
                            }
                            ...
                            try {
                                String value = data.get("test2").getAsString();
